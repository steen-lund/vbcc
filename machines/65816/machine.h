...
#define POINTER_VARADR(x) pointer_varadr(x,0)
extern int pointer_type();
extern int pointer_varadr();
#define ISPOINTER(x) ((x&NQ)>=POINTER&&(x&NQ)<=HLPOINTER)
#define ISSCALAR(x) ((x&NQ)>=CHAR&&(x&NQ)<=HLPOINTER)
#define ISINT(x) ((x&NQ)>=CHAR&&(x&NQ)<=LLONG)
#define PTRDIFF_T(x) ((x&NQ)!=POINTER?LONG:INT)
...
#define INLINEMEMCPY 0xFFFFFF

/*  Parameters are sometimes passed in registers without __reg.     */
#define HAVE_REGPARMS 1

/*  Assign the first register parameter last when optimizing        */
#define HAVE_REVERSE_REGPARMS 1
...
/* size_t depends on memory model */
#define HAVE_INT_SIZET (have_int_sizet_65816)
extern int have_int_sizet_65816;
...
#define ALLOCVLA_REG FIRST_GPR
#define ALLOCVLA_INLINEASM "\ttsc\n"\
                           "\tsec\n"\
                           "\tsbc\tr0\n"\
                           "\ttcs\n"\
                           "\tina\n"\
...
#define FREEVLA_INLINEASM "\ttcs"

#define OLDSPVLA_INLINEASM "\ttsc\n\tldx\t#0"
