...
static struct IC *preload(FILE *f,struct IC *p)
{
  ...
  if(p->q1.flags&&LOAD_STORE&&!isreg(q1)&&p->code!=CALL&&p->code!=SETRETURN){
...
#else
      emit(f,"\tadd.i\t%s%s%s,%ld\n",regnames[sp],THREE_ADDR?",":"",THREE_ADDR?regnames[sp]:"",zm2l(p->q2.val.vmax));
